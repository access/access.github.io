(function () {
    ...
    window.MSX_MOD.step = function (name, ctx, done) {
        ...
        (function () {
            ...
            // [ADDED] CUB blacklist mock response builders
            function makeJsonEmptyResponseFetch() {
                ...
                // fallback minimal "fetch-like" response
                return {
                    ok: true,
                    status: 200,
                    statusText: 'OK',
            ...
            function patchBlockNetwork() {
                if (window.fetch) {
                    ...
                    window.fetch = function (input, init) {
                        ...
                        // [ADDED] override CUB blacklist to empty list
                        if (isCubBlacklistUrl(u)) {
                            try { showOk('CUB', 'blacklist overridden', 'fetch | ' + String(u)); } catch (_) { }
                ...
                if (window.XMLHttpRequest) {
                    ...
                    XHR.prototype.open = function (method, url) {
                        ...
                        // [ADDED] override CUB blacklist to empty list (XHR)
                    ...
                    XHR.prototype.send = function () {
                        // [ADDED] mocked response for CUB blacklist
                        if (this.__ap_mock_cub_blacklist) {
                            ...
                            setTimeout(function () {
                                try {
                                    // define properties (часто readonly, но на инстансе обычно можно переопределить)
                                    ...
                                    try { showOk('CUB', 'blacklist overridden', 'XHR | ' + String(u0)); } catch (_) { }
                ...
                showOk('policy', 'Network block installed', 'Yandex + Google/YouTube + Statistics + BWA:CORS(/cors/check) + CUB:blacklist([])');
            }
            ...
            function start() {
                ...
                waitLampa(function (ok) {
                    ...
                    ensureInstalledAll(PLUGINS).then(function () {
                        ...
                        // [ADDED] auto reload после первой установки (как в reload.js: location.reload())
