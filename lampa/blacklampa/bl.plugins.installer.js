(function () {
  ...
  function pickSettingsName(cfg) {
    try {
      var s = getSettingsCfg(cfg);
      return String(s.pi.name || '') || 'Plugins Installer';
    } catch (_) { return 'Plugins Installer'; }
  }
  ...
  function isSettingsComponentOpen() {
    try {
      ...
      return cur === String(STATE.componentId || '');
    } catch (_) {
      return false;
    }
  }
    ...
    try {
      if (window.Lampa && Lampa.Modal && typeof Lampa.Modal.open === 'function' && window.$) {
        ...
        Lampa.Modal.open({
          ...
          buttons: [{
            name: 'Отмена',
            onSelect: function () { try { if (Lampa.Modal && Lampa.Modal.close) Lampa.Modal.close(); } catch (_) { } }
          }, {
            name: 'OK',
            onSelect: function () {
  ...
  function openInternal(route, payload, focusIndex, nav) {
    ...
    try {
      ...
      if (route !== 'root' && route !== 'managed' && route !== 'extras' && route !== 'plugin_detail' && route !== 'danger' &&
        route !== 'blocklist' && route !== 'blocklist_builtin' && route !== 'blocklist_user' && route !== 'blocklist_user_detail' && route !== 'blocklist_add') {
        route = 'root';
      }
